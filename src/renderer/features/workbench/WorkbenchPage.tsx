  ...
  return (
    <div id={`u-${p.id}`} className={cls} onClick={onSelect}>
      ...
      {isEditing ? (
        ...
        : p.final ? <div className={`unit-zh${p.final.confirmed ? ' confirmed' : ''}`} onDoubleClick={onEdit}><MarkedText text={p.final.text} ruby={p.final.ruby} />{p.final.notes?.map((note,index)=><p key={index} className="small muted" aria-label="译注">译注：{note}</p>)}</div>
