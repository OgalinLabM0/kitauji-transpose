...
export function TaskProgressView({ progress, state, live }: { progress: WorkflowProgress; state: SeriesDeliveryState | null; live: boolean }) {
  ...
  const translating = ['translate', 'trajectory', 'delivery'].includes(phase ?? '') || phase === 'knowledge' && !!run && run.total > 0 && run.done >= run.total;
  ...
  return <div className="task-progress-view">
    ...
    <div className="task-progress-heading"><strong>{done ? '成品已保存' : waiting ? '等待确认 · ' + state.waitingDecisionIds!.length + ' 项' : phaseLabel}</strong>
      {hasCount && !done && <span className="task-step-count">{detail.done.toLocaleString()} / {detail.total.toLocaleString()} {detail.unit}<b>{pct}%</b><small>当前步骤</small></span>}
