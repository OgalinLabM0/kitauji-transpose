...
export function scopedRegisters(store:ProjectStore,seriesId:string,paragraphs:{id:string;seriesOrdinal:number;paragraphType:string}[],speakerOf:(id:string)=>string|null|undefined):{paragraphId:string;characterId:string;name:string;value:string;origin:'user'|'model'}[] {
  ...
  for(const p of paragraphs){
    ...
    if(!facts){facts=store.db.all<CharacterFact>("SELECT character_id,field,value_json,valid_from_para,origin,evidence_ids,source_proof FROM character_field_history WHERE character_id=? AND field='speech_register' AND origin IN ('user','model') ORDER BY valid_from_para DESC",[speaker]);histories.set(speaker,facts);}
    const current=facts.filter(f=>characterFactFrom(store.db,f)<=p.seriesOrdinal && characterFactCurrent(store.db,f,speaker,'local'));
