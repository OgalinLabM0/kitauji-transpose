...
export class AppService {
  ...
  private latestProgress: WorkflowProgress = { running: false, paused: false, step: null, detail: null, phase: 'idle', done: 0, total: 0, currentParagraphId: null, costUsd: 0, inputTokens: 0, outputTokens: 0, message: '' };
  private lastLogId = 0;
  ...
  private resetMaintenanceUi(): string {
    ...
    this.latestProgress = { ...this.latestProgress, running: false, paused: false, step: null, detail: null, phase: 'idle', done: 0, total: 0, currentParagraphId: null, message: '' };
  ...
  private withTask<T>(work: () => Promise<T>): Promise<T> {
    ...
    return this.tasks.run(async () => {
      ...
      this.latestProgress = { ...this.latestProgress, running: true, paused: false, step: null, detail: null, done: 0, total: 0, phase: '准备任务', message: '准备任务…' };
      ...
      workflow: {
        ...
        continueSeries: sid => this.withTask(async () => {
          ...
          try { return await runSeriesFlow(this.store, this.ai, id.parse(sid), { signal: abort.signal, onState: state => {
            this.progress({ ...this.latestProgress, running: true, phase: '连续处理全部册', step: state.currentRun?.step ?? null, detail: state.currentRun?.detail ?? null, done: state.done, total: state.total, message: state.message, costUsd: 0, ...state.usage });
          } }); }
          finally { this.changed('paragraphs', 'queue', 'series', 'knowledge', 'glossary'); }
        }),
        ...
        continueVolume: (vid) => this.withTask(async () => {
          ...
          try { return await runVolumeFlow(this.store, this.ai, id.parse(vid), { signal: abort.signal, onState: state => {
            this.progress({ ...this.latestProgress, running: true, phase: '连续处理本册', step: state.step ?? null, detail: state.detail ?? null, done: state.done, total: state.total, message: state.message, costUsd: 0, inputTokens: state.usage?.inputTokens ?? 0, outputTokens: state.usage?.outputTokens ?? 0, unknownUsageRequests: state.usage?.unknownUsageRequests ?? 0 });
          } }); }
          finally { this.changed('paragraphs', 'queue', 'series', 'knowledge', 'glossary'); }
        }),
